defmodule EBOR do
  @moduledoc """
  The Concise Binary Object Representation (EBOR) is a data format
  whose design goals include the possibility of extremely small code
  size, fairly small message size, and extensibility without the need
  for version negotiation.  These design goals make it different from
  earlier binary serializations such as ASN.1 and MessagePack.

  The objectives of EBOR, roughly in decreasing order of importance are:
  ...
  that can be interpreted by other EBOR libraries.

  ## Examples

      iex> EBOR.encode(["Hello", "World!"])
      <<130, 101, 72, 101, 108, 108, 111, 102, 87, 111, 114, 108, 100, 33>>

      iex> EBOR.encode([1, [2, 3]])
      <<130, 1, 130, 2, 3>>

      iex> EBOR.encode(%{"a" => 1, "b" => [2, 3]})
  ...
  @doc """
  Converts a EBOR encoded binary into native elixir data structures

  ## Examples

      iex> EBOR.decode(<<130, 101, 72, 101, 108, 108, 111, 102, 87, 111, 114, 108, 100, 33>>)
      {:ok, ["Hello", "World!"], ""}

      iex> EBOR.decode(<<130, 1, 130, 2, 3>>)
      {:ok, [1, [2, 3]], ""}

      iex> EBOR.decode(<<162, 97, 97, 1, 97, 98, 130, 2, 3>>)
      {:ok, %{"a" => 1, "b" => [2, 3]}, ""}
  ...
  defp perform_decoding(binary) when is_binary(binary) do
    case EBOR.Decoder.decode(binary) do
